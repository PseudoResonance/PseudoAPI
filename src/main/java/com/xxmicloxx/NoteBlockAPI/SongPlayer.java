...
public abstract class SongPlayer {
	...
	private void start() {
		plugin.doAsync(() -> {
			while (!destroyed) {
				...
				lock.lock();
				try {
					if (destroyed || WolfAPI.plugin.isDisabling()) {
						break;
					}
	...
	public void addPlayer(Player p) {
		...
		try {
			if (!playerList.containsKey(p.getName())) {
				...
				ArrayList<SongPlayer> songs = WolfAPI.plugin.playingSongs.get(p.getName());
				if (songs == null) {
					songs = new ArrayList<SongPlayer>();
				}
				songs.add(this);
				WolfAPI.plugin.playingSongs.put(p.getName(), songs);
			}
		} finally {
			lock.unlock();
		}
	...
	public void removePlayer(Player p) {
		...
		try {
			...
			if (WolfAPI.plugin.playingSongs.get(p.getName()) == null) {
				return;
			}
			ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(WolfAPI.plugin.playingSongs.get(p.getName()));
			songs.remove(this);
			WolfAPI.plugin.playingSongs.put(p.getName(), songs);
